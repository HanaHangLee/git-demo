import os
import torch
from tqdm import tqdm #Khi bạn sử dụng tqdm để bao bọc một vòng lặp, nó sẽ tự động tạo ra một thanh tiến trình để hiển thị tiến trình của vòng lặp đó. 
...
class Trainer:
    ...
    def _set_ddp_training(self):
        """ Initialize the DistributedDataParallel wrapper for the model. You would need to pass the model and specify the device IDs and output device for the data parallelism."""
    ...
    def prepare_dataloader(self, train_dataset, eval_dataset):
        ...
        data_testloader = DataLoader(eval_dataset, batch_size=self.batch_size, sampler=eval_sampler, collate_fn=data_collator, drop_last=True)
...
def load_pretrained_model(local_rank, model_path: str = ""):
    ...
    lora_config = LoraConfig(r=4, lora_alpha=8, lora_dropout=0.05, bias="none", task_type="CAUSAL_LM", target_modules=['lm_head.linear', 'transformer.embd.wte'])
